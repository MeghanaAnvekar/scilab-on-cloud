...
class ScilabInstance(object):
	...
	def spawn_instance(self):	
		if (self.count < self.maxsize):
			...
			try:
				new_instance.expect('-->', timeout = 120)
			except:
				print "Exception Occured: ", sys.exc_info()[0]
			self.count += 1
	
	# killing some spawned(unused) instances
	...
	def execute_code(self, code, token, book_id, dependency_exists):
		...
		try:
			active_instance.expect('\[0m ', timeout = 120)
			active_instance.expect('', timeout = 120)
		except:
			print "Exception Occured: ", sys.exc_info()[0]
			active_instance.before += str("Exception Occured: ") + str(sys.exc_info()[0])
